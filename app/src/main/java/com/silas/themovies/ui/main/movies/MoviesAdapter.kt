...
import com.silas.themovies.model.type.PosterPath
 ...
 */
class MoviesAdapter(private var listMovies: ArrayList<Movie> = arrayListOf(),
                    private var isRelated: Boolean = false,
                    private val onClickMovie: ((movieId: Movie) -> Unit)? = null)
    : RecyclerView.Adapter<MoviesAdapter.MovieViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MovieViewHolder {
        val layoutId = if (isRelated) R.layout.item_movie_related else R.layout.item_movie
    ...
    override fun onBindViewHolder(holder: MovieViewHolder, position: Int) {
        listMovies[position].let { itMovie ->
            holder.itemView.apply {
                ...
                val endPoint =
                    if (isRelated) BackDrop.W780(itMovie.backdropPath).endPoint
                    else PosterPath.W780(itMovie.posterPath).endPoint

                image_view_movie.setUpImage(endPoint, circularProgressDrawable)
